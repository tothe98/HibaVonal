...
export default function OrdersClientPage({ user }: Props) {
  ...
  const handleStatusChange = async (order: Order, newStatusValue: number) => {
    ...
    setOrders((prev) => prev.map((o) => (o.id === order.id ? { ...o, status: newLabel } : o)));
  };
              ...
              sortedOrders.map((order) => {
                const handleMouseEnter = (e: React.MouseEvent<HTMLSpanElement>) => {
                  ...
                  if (popup) {
                    ...
                    // Check if there's more space below or above and position accordingly
                    if (spaceBelow >= popup.offsetHeight || spaceBelow > spaceAbove) {
                ...
                return (
                  <tr key={order.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="p-2">
                      <select value={statusOptions.find((opt) => opt.label === order.status)?.value ?? ""} onChange={(e) => handleStatusChange(order, parseInt(e.target.value))} className="border rounded px-2 py-1">
                    ...
                    <td className="p-2 relative group">
                      ...
                      <div className="fixed hidden group-hover:block bg-white border border-gray-300 rounded-lg shadow-lg p-4 z-50 min-w-[350px] max-h-[300px] overflow-y-auto">
