...
import { fetchMaintenanceWorkers, fetchReports, updateIssue } from "./actions";
...
export default function ReportsClientPage({ user }: Props) {
    ...
    useEffect(() => {
        ...
        const loadWorkers = async () => {
            const result = await fetchMaintenanceWorkers();
            if (!result.success) {
                setError(result.error);
                return;
            }
    ...
    const handleStatusChange = async (issue: Issue, newStatusValue: number) => {
        ...
        setIssues((prev) => prev.map((i) => (i.id === issue.id ? { ...i, status: newLabel } : i)));
    };

    const handleWorkerChange = async (issue: Issue, newWorkerId: number) => {
        const status: number = statusOptions.find((opt) => opt.label === issue.status)?.value ?? 0;
        const result = await updateIssue(issue.id, status, newWorkerId);

        if (!result.success) {
            setError(result.error);
            return;
        }
        const selectedWorker = workers.find((w) => w.id === newWorkerId) || null;

        setIssues((prev) => prev.map((i) => (i.id === issue.id ? { ...i, maintenanceWorker: selectedWorker } : i)));
    };
    ...
    const sortedIssues = useMemo(() => {
        ...
        return [...issues].sort((a, b) => {
            ...
            if (sortKey === "maintenanceWorker") {
                aVal = a.maintenanceWorker?.name || "";
                bVal = b.maintenanceWorker?.name || "";
            } else {
                aVal = a[sortKey] as any;
                bVal = b[sortKey] as any;
            }
    ...
    return (
        <main
            ...
            `}
        >
            <h1 className="text-2xl font-semibold sm:mb-0 mb-4">Reports</h1>
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="text-gray-700 border-b">
                            ...
                            {renderHeader("Worker", "maintenanceWorker")}
                        ...
                        {sortedIssues.length === 0 ? (
                            ...
                            sortedIssues.map((issue) => (
                                <tr key={issue.id} className="border-t hover:bg-gray-50">
                                    ...
                                    <td className="p-2">
                                        <select value={statusOptions.find((opt) => opt.label === issue.status)?.value ?? ""} onChange={(e) => handleStatusChange(issue, parseInt(e.target.value))} className="border rounded px-2 py-1">
                                    ...
                                    <td className="p-2">
                                        <select value={issue.maintenanceWorker?.id ?? ""} onChange={(e) => handleWorkerChange(issue, parseInt(e.target.value))} className="border rounded px-2 py-1">
